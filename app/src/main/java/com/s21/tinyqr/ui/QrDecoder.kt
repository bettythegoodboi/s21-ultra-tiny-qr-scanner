...
object QrDecoder {
    ...
    private val dmOnly: EnumSet<BarcodeFormat> = EnumSet.of(BarcodeFormat.DATA_MATRIX)
    private val all2d: EnumSet<BarcodeFormat> = EnumSet.of(
    ...
    private fun makeHints(formats: EnumSet<BarcodeFormat>): Hashtable<DecodeHintType, Any> {
    ...
    private fun decodeMulti(bitmap: Bitmap): String? {
        return try {
            ...
            for (formats in listOf(dmOnly, all2d)) {
                reader.setHints(makeHints(formats))
                try {
                    val r = reader.decodeWithState(BinaryBitmap(HybridBinarizer(source)))
                    Log.d("TinyQR", "Decoded ${r.barcodeFormat}: ${r.text}")
                    return r.text
                } catch (_: Exception) {
                }
                try {
    ...
    private fun tryImage(bmp: Bitmap): String? {
        ...
        decodeDataMatrixDirect(c1)?.let { c1.recycle(); return it }
        decodeMulti(c1)?.let { c1.recycle(); return it }

        val c2 = contrastGray(bmp, 2.2f)
        decodeDataMatrixDirect(c2)?.let { c1.recycle(); c2.recycle(); return it }
        decodeMulti(c2)?.let { c1.recycle(); c2.recycle(); return it }

        val inv = invert(c1)
        decodeDataMatrixDirect(inv)?.let {
